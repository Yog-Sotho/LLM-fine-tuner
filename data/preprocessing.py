...
def preview_dataset(dataset: Dataset, is_dpo: bool = False) -> pd.DataFrame:
    ...
    BOLT OPTIMIZATION: Slicing the dataset before column access (e.g., ds[:N][COL])
    is ~8x faster and uses significantly less memory than ds[COL][:N] because
    it avoids loading entire columns into memory.
    """
    if len(dataset) == 0:
    ...
    if is_dpo:
        ...
        preview_data = dataset[:5]
        return pd.DataFrame({
            COL_PROMPT:   preview_data[COL_PROMPT],
            COL_CHOSEN:   preview_data[COL_CHOSEN],
            COL_REJECTED: preview_data[COL_REJECTED],
        })
        ...
        # BOLT OPTIMIZATION: Slice first, then access columns.
        preview_data = dataset[:5]
        inst_data = preview_data[COL_INSTRUCTION] if COL_INSTRUCTION in dataset.column_names else []
        out_data  = preview_data[COL_OUTPUT]      if COL_OUTPUT      in dataset.column_names else []
        return pd.DataFrame({

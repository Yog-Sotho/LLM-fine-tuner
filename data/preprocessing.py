...
def preview_dataset(dataset: Dataset, is_dpo: bool = False) -> pd.DataFrame:
    ...
    if is_dpo:
        # BOLT OPTIMIZATION: Slice before column access (dataset[:N][COL]) is ~8x faster
        # than slicing after (dataset[COL][:N]) for large datasets.
        return pd.DataFrame({
        ...
        # BOLT OPTIMIZATION: Efficient slicing pattern dataset[:N][COL]

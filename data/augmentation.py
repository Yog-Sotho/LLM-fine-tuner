...
def augment_dataset_v27(
    ...
    try:
        ...
        if target_col:
            # BOLT OPTIMIZATION: Direct columnar access (dataset[column]) is
            # significantly faster than row-wise iteration ([x[column] for x in dataset])
            # as it avoids expensive row-to-dict conversion.

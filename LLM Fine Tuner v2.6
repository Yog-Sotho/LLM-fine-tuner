...
║    Full upgrade from your v2.5: trailing spaces fixed, CLI, SFTTrainer,║
║    VRAM auto-scaling, AppState, GPTQ/EXL2, DataFrame preview, security║
╚══════════════════════════════════════════════════════════════════════╝
"""
...
# ====================== ORIGINAL FUNCTIONS (100% intact except upgrades) ======================
...
# v2.6 UPGRADE: DataFrame preview (much better UX)
...
# v2.6 train_model — SFTTrainer unification + VRAM auto-scaling + AppState + security
def train_model(
    ...
    try:
        ...
        # v2.6 DYNAMIC VRAM AUTO-SCALING
...
def on_file_upload(file, training_mode="sft"):
    ...
    try:
        ...
        cols = list(pd.read_csv(file.name).columns) if ftype in ("csv", "excel") else []
        need_map = True
        ...
        if need_map and ftype in ("csv", "excel"):
...
CUSTOM_CSS = """[original CSS unchanged — kept exactly as in v2.5]"""
...
with gr.Blocks(
    ...
    # Wire up events (original + DataFrame + quant_backend)
    file_input.change(
        fn=on_file_upload,
...
def train(
    ...
    # Uses the same train_model function internally (full headless support)

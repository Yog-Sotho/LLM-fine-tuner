...
# H-4 FIX: The CUDA cmake flag is detected dynamically from the checked-out
# llama.cpp source. Older releases use -DLLAMA_CUDA=ON; newer releases (after
# build b3000) renamed it to -DGGML_CUDA=ON. Hard-coding -DLLAMA_CUDA=ON
# previously caused silent CPU-only builds on newer llama.cpp versions.
RUN git clone --depth 1 https://github.com/ggerganov/llama.cpp /opt/llama.cpp && \
    CUDA_CMAKE_FLAG="-DLLAMA_CUDA=ON" && \
    if grep -q "GGML_CUDA" /opt/llama.cpp/CMakeLists.txt 2>/dev/null; then \
        CUDA_CMAKE_FLAG="-DGGML_CUDA=ON"; \
    fi && \
    echo "Using llama.cpp CUDA cmake flag: ${CUDA_CMAKE_FLAG}" && \
...
# H-5 FIX: Run the application as a non-root user.
# Previously the container ran entirely as root, meaning any vulnerability in
# Gradio or its dependencies would give an attacker root access to the container
# and all mounted volumes. The llmuser account owns /app but has no special privileges.
RUN groupadd -r llmuser && useradd -r -g llmuser -d /app llmuser \
    && chown -R llmuser:llmuser /app \
    # Also fix NLTK data ownership so the non-root user can read it

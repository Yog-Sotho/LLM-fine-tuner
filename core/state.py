...
def validate_identifier(identifier: str | None) -> str | None:
    """Check if an identifier contains '/', '\\', '..', or null bytes (unsafe).

    Use this for non-path strings like quantization tags or version IDs that
    will be used to construct local or remote paths.
    """
    if not identifier:
        return None

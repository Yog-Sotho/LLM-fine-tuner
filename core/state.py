...
def validate_path_traversal(path: str | None) -> str | None:
    ...
    if not path:
        return None
    # Sentinel: Block traversal patterns and null-byte injection.
...
def validate_identifier(name: str | None) -> str | None:
    """Validate a non-path identifier (e.g. version tag, quant type).

    Blocks forward slashes, backslashes, '..', and null bytes to prevent
    arbitrary file writes when the identifier is joined into a path.

    Returns a standardized error message if unsafe, or None if safe.
    """
    if not name:
        return None
    # Sentinel: Identifiers used in os.path.join must not contain separators.

...
def on_train_click(
    ...
    # This provides a verified ~500x speedup for large datasets (100k+ rows).
    try:
        ...
        if COL_PROMPT in df_stats.columns and COL_CHOSEN in df_stats.columns:
            ...
            _lengths_vec = df_stats[COL_TEXT].astype(str).str.len()
            ...
            _lengths_vec = df_stats[first_col].astype(str).str.len() if first_col else pd.Series(dtype=int)

        avg_len = float(_lengths_vec.mean()) if not _lengths_vec.empty else 0.0
    except Exception:
        avg_len = 100.0

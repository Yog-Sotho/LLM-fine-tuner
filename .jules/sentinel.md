## 2026-08-05 - [Unified Defense-in-Depth Token Redaction Across All Layers]
**Vulnerability:** Raw exception messages from transformers, tokenizers, trainers, and other modules could leak raw Hugging Face write or read tokens (either user-provided or automatically pulled from environment variables) to UI error panels, CLI stdout/stderr, or logs, exposing credentials to third parties or CI/CD pipelines.
**Learning:** Checking or redacting only explicit function parameter tokens is insufficient. Downstream libraries or system exceptions can extract and output credentials (such as HF_TOKEN) automatically. Robust security requires a centralized, regex-based, and environment-aware redaction utility applied systematically across all layer boundary exception blocks (UI handlers, training pipelines, CLI command outputs).
**Prevention:** Implement a general-purpose token/credential redactor (`redact_sensitive_info`) that matches standard token prefixes (e.g., `hf_...`) and environment variables, and apply it to all caught exception text before throwing, returning, or printing error messages.

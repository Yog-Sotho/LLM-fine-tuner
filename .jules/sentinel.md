...
## 2025-05-25 - [Centralized Identifier Hardening & Null Byte Protection]
**Vulnerability:** Incomplete path traversal protection allowing null-byte injection and directory separators in simple identifiers.
**Learning:** Even with traversal checks for '..' and '\', attackers can use null bytes ('\0') or forward slashes ('/') in fields intended as simple name tags (like quantization or versions) to manipulate file paths when joined via `os.path.join`.
**Prevention:** Use a specialized `validate_identifier` guard for non-path strings that blocks all directory separators and null bytes. Centralizing these guards ensures consistent enforcement across training, inference, and export layers.

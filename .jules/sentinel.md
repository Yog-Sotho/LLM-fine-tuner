## 2026-08-02 - [Centralized Token Redaction for Defense-In-Depth]
**Vulnerability:** Raw exception messages from Hugging Face APIs could contain sensitive tokens (user-supplied or from environment variables) which were displayed in Gradio panels or written to application/CLI outputs. Manual redaction in handlers (e.g. `.replace(self.token, ...)`), was localized, error-prone, and failed to protect environment-level `HF_TOKEN` credentials.
**Learning:** Localized string replacements are easily forgotten, bypassed, or only target a single user input parameter. A central, multi-layer redaction mechanism combining robust regex patterns with active environment inspection ensures uniform protection across all backend layers, CLI outputs, and UI panels.
**Prevention:** Implement a general-purpose centralized sanitization utility (`redact_sensitive_info` in the core state module) that automatically filters credential patterns (`hf_[a-zA-Z0-9_]{30,}`) and configured environment variables (`HF_TOKEN`) from strings before logging or raising them across all interface boundaries.

...
## 2026-06-12 - [Vectorized Data Augmentation & Slicing]
**Learning:** Row-wise iteration for text extraction and dictionary construction in 'augment_dataset_v27' is a massive bottleneck, especially for larger datasets. Switching to columnar access 'list(dataset[COL])' and vectorized Pandas reconstruction (pd.concat + sort_index) yields a ~64x speedup. Furthermore, 'dataset[COL][:N]' loads the entire column into memory before slicing, whereas 'dataset[:N][COL]' only loads the requested slice, providing a ~6x speedup for 1M-row previews.
**Action:** Optimized 'augment_dataset_v27' with Pandas-based reconstruction and updated 'preview_dataset' to use the more efficient slicing pattern. Added 'tests/benchmark_bolt_augmentation.py' and 'tests/benchmark_bolt_extraction.py' for verification.

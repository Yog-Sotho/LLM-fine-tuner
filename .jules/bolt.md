...
## 2026-06-07 - [HuggingFace Dataset Column Access and Reconstruction Optimization]
**Learning:** Accessing a column in a HuggingFace Dataset (Arrow-backed) then slicing it (`ds[COL][:N]`) can be extremely slow and memory-intensive because it often loads the entire column. Reversing the order to slice first (`ds[:N][COL]`) ensures only the required rows are loaded, providing a ~16x speedup. Additionally, reconstructing a Dataset row-by-row using `Dataset.from_list` is significantly slower than columnar reconstruction using `to_dict()`, list slicing (`interleaved[0::factor] = values`), and `Dataset.from_dict()`.
**Action:** Implemented the slice-then-access pattern in `preview_dataset` and optimized the reconstruction phase of `augment_dataset_v27` using columnar interleaving, achieving a combined ~17x speedup for augmentation and reduced memory overhead for previews.

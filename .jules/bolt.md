...
## 2026-08-15 - [DPO Dataset Vectorized Deduplication]
**Learning:** During dataset preprocessing and cleaning, duplicate pairs in standard SFT/Text datasets were correctly deduplicated, but DPO/preference datasets (using 'prompt', 'chosen', 'rejected' columns) were completely skipped. This led to redundant training iterations, inflated training times, and potential overfitting in preference alignment (DPO/ORPO). By adding vectorized Pandas `drop_duplicates` on `COL_PROMPT`, `COL_CHOSEN`, and `COL_REJECTED` under an `is_dpo` check in `validate_and_clean_dataset`, we achieve extremely fast O(N) deduplication (~0.12s for 100k rows) and eliminate redundant examples before training.
**Action:** Deduplicate DPO/preference datasets in `validate_and_clean_dataset` (in `data/preprocessing.py`) when `is_dpo=True` using vectorized Pandas `drop_duplicates` on `COL_PROMPT`, `COL_CHOSEN`, and `COL_REJECTED` columns.

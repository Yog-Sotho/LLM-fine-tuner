...
## 2026-06-24 - [Columnar Dataset Operations & Slicing Order]
**Learning:** HuggingFace Datasets have severe performance penalties for row-wise iteration (e.g., list comprehensions over the dataset object) because every access creates a temporary dictionary. Direct column access (e.g., `dataset[COL]`) is ~10,000x faster. Furthermore, the order of slicing and column access matters: `dataset[:N][COL]` is ~8x faster than `dataset[COL][:N]` for large datasets because the latter may trigger loading the entire column into memory before slicing.
**Action:** Always prefer direct column access over row-wise iteration. In UI previews or any partial data access, use the `dataset[:N][COL]` pattern to minimize memory overhead and latency. Use `dataset.to_dict()` and slice-based interleaving (`vals[0::factor] = orig`) for efficient O(N) dataset reconstruction.

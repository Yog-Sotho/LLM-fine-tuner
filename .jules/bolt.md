...
## 2026-06-18 - [Dataset Augmentation & Preview Indexing Optimization]
**Learning:** Sequential row-wise dictionary creation during dataset reconstruction (post-augmentation) is a major O(N * factor) bottleneck. Transitioning to columnar interleaving via `dataset.to_dict()` and slice-assignment (`expanded[0::factor] = values`) yields a verified ~15x speedup for the reconstruction phase on 10k rows. Additionally, HuggingFace Datasets are significantly more efficient when sliced before column access (`dataset[:N][COL]`) as it avoids loading entire columns into memory.
**Action:** Implemented columnar reconstruction in `data/augmentation.py:augment_dataset_v27` and optimized slicing in `data/preprocessing.py:preview_dataset`. Added `tests/benchmark_augmentation_reconstruction.py` for verification.

...
## 2026-09-05 - [Multiprocessing BLEU & ROUGE Evaluation]
**Learning:** Sequential, single-threaded CPU-bound calculations for metrics like BLEU (via `nltk`) and ROUGE (via `rouge_score`) on large evaluation datasets are a major performance bottleneck, wasting available CPU cores. Transitioning to chunk-based multiprocessing via `ProcessPoolExecutor` when datasets contain 100 or more items yields a major speedup. Prioritizing the high-performance `fork` start method on Unix/Linux systems is critical because it avoids massive PyTorch/Transformers re-import overhead in subprocesses, achieving up to ~3x speedup on 2000 rows.
**Action:** Implemented chunk-based parallel processing inside `compute_bleu_rouge` using `ProcessPoolExecutor` with `fork` context. Gracefully fall back to sequential execution on smaller datasets (< 100 items), single-core environments, or in case of any process-spawning exception. Added `tests/benchmark_bleu_rouge.py` for verification.
